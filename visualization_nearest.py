...
def get_embs(traj_path,model,_transforms,args,device):
...
def knn_cosine_estimate(embs_all, labels_all, test_embs, K):
    # 1. L2 normalization
...
def main(ckpt,args):
    ...
    labels_all = []
    embs_all = []
    trajs_path = glob.glob(os.path.join(data_path,'vid*'))
    #num_val_trajs = 10-num_train_trajs

    for traj_path in tqdm(trajs_path[:-1]):
        # read images
        # trajs_path = os.path.join(data_path,f'vid{i}')
        # print(trajs_path)
        a_emb_reduced = get_embs(traj_path,model,_transforms,args,device)
    ...
    #test
    test_embs = get_embs(trajs_path[-1],model,_transforms,args,device)
    estimated = knn_cosine_estimate(embs_all,labels_all,test_embs,3)
